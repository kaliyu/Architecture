...
public class Facade {
   ...
   public Collection<Message> getReceivedMessage(String login){
    ...
    public void delete(String login) {
        MyUser user=em.find(MyUser.class,login);
        if (user!=null) {
            em.createQuery("DELETE FROM Message m WHERE m.to = :user") .setParameter("user", user) .executeUpdate();
            em.createQuery("DELETE FROM Message m WHERE m.from = :user") .setParameter("user", user) .executeUpdate();
        }
    }
